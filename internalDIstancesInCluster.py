...
##        print angle_distance_sum
    ...
    def Distance(self):
        if (self.size > 1):
            for part1 in self.__particleList:
                for part2 in self.__particleList[1:]:
                    return part1[0].pos-part2[0].pos

    def Shift(self):
        if (self.size > 1):
            for part1 in self.__particleList:
                for part2 in self.__particleList[1:]:
                    return part1[0].pos.ScalarProduct(part1[0].dir)-part2[0].pos.ScalarProduct(part1[0].dir);
                
    def AngleBetween(self):
        if (self.size > 1):
            for part1 in self.__particleList:
                for part2 in self.__particleList[1:]:
#                    return part1[0].dir.ScalarProduct(part2[0].dir)
                    angle=57.29577951*math.acos(part1[0].dir.ScalarProduct(part2[0].dir));
  #                  return angle
                    if (angle > 90.0):
                        return 180.0-angle
                    else:
...
#            print line_parts[i+1]
    ...
    for sweep in clusters_all:
        ...
        for cluster in sweep[1:]:
            if(cluster.size > 1 ):
                print cluster.Distance().norm(),cluster.Shift(), cluster.AngleBetween()
